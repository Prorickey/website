...
const dockerBuildChild = spawn('docker', ['build', '-t', `docker.prorickey.xyz/prorickey/nextjs-portfolio:${version}`, '.']);
...
dockerBuildChild.on('exit', (code) => {
    ...
    const dockerPushChild = spawn('docker', ['push', `docker.prorickey.xyz/prorickey/nextjs-portfolio:${version}`]);
